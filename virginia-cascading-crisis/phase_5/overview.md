# Phase 5: Hidden Damage

**Course Topic:** Health, Environmental, & Biosecurity

**Time Period:** Day 16-25 of crisis (60 minutes real-time)

---

## Context

Phase 4 held the line on governance and civil unrest — but the costs were invisible. In the rush to manage protests and deepfakes, critical monitoring systems stayed offline. Environmental sensors were never restarted. Rural health infrastructure remained fragmented. The coalfields were stabilized politically, but nobody checked what was happening underneath.

Now the hidden damage surfaces all at once.

Carilion's rural clinics — already understaffed before the cyber attack — are running on fumes. Dialysis patients who survived two weeks of disruption are now at immediate risk: four rural clinics are offline, and the transport gaps cannot hold. Medication supplies for opioid recovery patients are running short, and overdose calls have spiked 40% in three days. The healthcare system that barely held together during Phase 4 is now facing collapse on multiple fronts simultaneously.

The environmental picture is just as alarming. Environmental sensors restored after the attack are sending readings nobody is ready for. A coal ash pond in Wise County shows potential structural issues before a major rain event. Three rural water systems — operated on manual backup for two weeks — show elevated contamination markers in preliminary tests. A coal processing facility that restarted without monitoring now sits at the center of a respiratory illness cluster affecting 47 residents in a coal community.

These are not rumor or political narrative problems. They are evidence-based crises — but the evidence is incomplete, the decisions are irreversible in both directions, and the public is watching. Premature advisories could trigger panic and supply crises. Delayed action could cause preventable deaths and long-term health damage that haunts the region for decades.

Teams must triage healthcare while simultaneously assessing environmental risks and managing communication about what is known, what is unknown, and what is being done.

---

## Crisis Engine Analysis

### What just changed unexpectedly?

Hidden environmental and health damage that was masked by Phase 4's governance crisis has surfaced simultaneously. The region is dealing with health triage + environmental risk + communication demands all at once — with no additional resources and exhausted teams.

### Which system is now stressed?

- [x] Health: dialysis, MOUD, hospital surge capacity, rural clinic collapse
- [x] Environment: coal ash integrity, water contamination, air quality, mine restart
- [x] Information: incomplete data driving high-stakes decisions; premature disclosure risk
- [ ] Infrastructure
- [ ] Governance (stabilized but fragile)

### Who experiences immediate harm?

- [x] Individuals: dialysis patients, opioid recovery patients, residents drinking contaminated water, coal community with respiratory illness
- [x] Vulnerable communities: rural elderly in hollows without transport, low-income families without safe water alternatives
- [x] Institutions: Carilion (capacity crisis), DEQ (evidence and authority gaps), faith/community networks (overwhelmed)
- [ ] Cross-border populations

### What decision must be made within limited time?

**Triage vs. transparency vs. precaution.** Teams must decide which health emergencies to prioritize with limited resources, how quickly to issue environmental advisories with incomplete data, and how to communicate risks without triggering panic that worsens supply chain and community stability.

**Deadline:** Dialysis patients require treatment every 48 hours — deaths are preventable only if transport is arranged within the first 20 minutes of play. Coal ash structural window closes before the forecast rainfall event within the first 15 minutes.

### What future risk is created by this decision?

- Delayed dialysis transport: preventable deaths, federal scrutiny, legal exposure
- Premature water advisory: panic buying, supply chain disruption, loss of DEQ credibility
- Missed environmental enforcement: long-term contamination, mine restart liability
- Over-medicalizing the respiratory cluster without evidence: creates biosecurity panic; under-responding: outbreak grows
- Insufficient MOUD support: community relapse crisis with multigenerational consequences

### Who makes the decision, and who is left out?

- Decision makers: DEQ, Carilion, Governor's Liaison, EMA, VT Research
- Stakeholders excluded: rural residents without transportation, opioid recovery community without political voice, coal workers dependent on the mine reopening
- Why this matters: the excluded groups bear the greatest harm from both action and inaction, and their exclusion from decision-making will shape Phase 6 community trust.

---

## Team Roles This Phase

**All 15 teams are active with health/environmental focus.** Budgets and trust carry forward from Phase 4 end state.

| Team | Role | Primary Focus This Phase |
|------|------|--------------------------|
| Acadia | VA DEQ Emergency Environmental Response | Coal ash, water testing, air quality, environmental emergency |
| Arches | Carilion Health System Crisis Command | Dialysis triage, hospital mutual aid, MOUD clinic support |
| Banff | Governor's Health Emergency Coordinator | Health emergency declarations, state resource authorization |
| Bryce | Appalachian Faith Network Community Health | Vulnerable population outreach, MOUD peer support, harm reduction |
| Denali | VDOT Medical Transport & Supply Routes | Patient transport corridors, pharmaceutical supply routes |
| Glacier | SW Virginia Mass Casualty Coordination | Mass casualty protocols, multi-agency medical coordination |
| Jasper | Public Health Biosecurity Coordinator | Biosecurity assessment, CDC liaison, outbreak risk analysis |
| Olympic | Regional Medical & Food Supply Chain | Pharmaceutical supply chain, emergency water distribution |
| Redwood | United Way Health Navigation Services | MOUD patient support, health navigator deployment |
| Sequoia | Virginia Tech Environmental Health Research | Respiratory cluster investigation, environmental data analysis |
| Shenandoah | Montgomery County Public Health Authority | Local public health directives, water advisory implementation |
| Yellowstone | WDBJ7 Health & Environmental Coverage | Health communications, environmental transparency coverage |
| Yoho | Shentel Emergency Health Communications | Telehealth bandwidth, health alert systems, crisis comms |
| Yosemite | SW Virginia Sheriff's Environmental Crime Unit | Environmental enforcement, mine oversight, security |
| Zion | AEP Power Grid & Environmental Systems | Power to healthcare facilities; coal ash pond infrastructure |

### Team-Specific Objectives

**Acadia — VA DEQ Emergency Environmental Response:**
- Complete coal ash structural assessment and decide on emergency stabilization before the rain event
- Issue or delay water advisory based on confirmatory testing vs. precautionary principles
- Investigate respiratory cluster air quality data in coordination with Sequoia (VT) and Jasper (Biosecurity)

**Arches — Carilion Health System Crisis Command:**
- Coordinate emergency dialysis transport and temporary clinic solutions for 340+ patients
- Activate hospital mutual aid to manage patient transfer volume
- Support MOUD clinic emergency dispensing to prevent withdrawal crises

**Banff — Governor's Health Emergency Coordinator:**
- Declare public health emergency to unlock mutual aid and emergency funding
- Authorize state-level support for dialysis transport and pharmaceutical resupply
- Coordinate federal engagement (CDC, EPA) and lead the public response to preventable deaths

**Bryce — Appalachian Faith Network Community Health:**
- Reach elderly and homebound residents in hollows without internet or phone access
- Deploy peer support and harm reduction outreach to opioid recovery patients at risk of seeking street alternatives
- Coordinate faith-based sites as emergency medication pickup and safe haven locations

**Denali — VDOT Medical Transport & Supply Routes:**
- Designate and operationalize patient transport corridors to regional dialysis and emergency care facilities
- Support pharmaceutical supply route planning for emergency medication depots
- Coordinate with Glacier (EMA) on mass casualty transport capacity

**Glacier — SW Virginia Mass Casualty Coordination:**
- Stand up mass casualty coordination protocols for potential dialysis, respiratory, and overdose surges
- Coordinate multi-agency medical response across county lines
- Manage public alerts for water, air, and health advisories in coordination with Acadia (DEQ)

**Jasper — Public Health Biosecurity Coordinator:**
- Assess respiratory cluster against biosecurity escalation criteria
- Coordinate CDC emergency consultation request if escalation criteria are met
- Produce situation assessment clarifying normal outbreak vs. biosecurity concern for decision-makers

**Olympic — Regional Medical & Food Supply Chain:**
- Coordinate emergency pharmaceutical supply from regional distribution networks using cold-chain logistics
- Manage emergency bottled water distribution if water advisory is issued
- Ensure supply route integrity for medical and water distribution operations

**Redwood — United Way Health Navigation Services:**
- Deploy health navigators to opioid recovery community to prevent relapse cascade
- Connect isolated rural patients to emergency transportation and pharmacy support
- Document and escalate unmet health needs that fall through agency gaps

**Sequoia — Virginia Tech Environmental Health Research:**
- Lead epidemiological investigation of the Appalachia respiratory cluster
- Provide real-time environmental data analysis (air quality, particulate sampling) to support Acadia (DEQ)
- Deploy environmental monitoring to close data gaps left by weeks of offline sensors

**Shenandoah — Montgomery County Public Health Authority:**
- Make and implement the water advisory decision for affected rural systems
- Coordinate with Acadia (DEQ) on confirmatory testing timing and interim advisories
- Host transparent community forums to explain environmental risks with what is known and unknown

**Yellowstone — WDBJ7 Health & Environmental Coverage:**
- Coordinate health and environmental communications to prevent panic while ensuring informed public
- Cover respiratory cluster and water advisory decisions with appropriate scientific context
- Counter premature social media contamination and outbreak claims with verified reporting

**Yoho — Shentel Emergency Health Communications:**
- Expand telehealth bandwidth for remote patient consultations (dialysis, MOUD follow-ups)
- Stand up health alert systems, public hotlines, and SMS notification for water and air advisories
- Ensure connectivity to rural health sites for crisis communications and monitoring

**Yosemite — SW Virginia Sheriff's Environmental Crime Unit:**
- Enforce environmental emergency orders (mine operations halt, coal ash pond access restrictions)
- Secure pharmaceutical supply depots against theft during shortage conditions
- Manage protest and worker walkout situation at mine gate if operations are halted

**Zion — AEP Power Grid & Environmental Systems:**
- Maintain power to healthcare facilities and dialysis sites as priority load
- Provide infrastructure access and technical support for coal ash pond emergency assessment
- Ensure environmental sensor systems remain powered and transmitting

---

## Injects

Injects are stored in `injects.csv`

### Initial Injects (5 total)

Each initial inject requires collaboration of exactly 3 teams. All 15 teams are covered.

| # | Category | Teams (Phase 5 Roles) | Description |
|---|----------|----------------------|-------------|
| 1 | Health/Transport | Arches (Carilion), Glacier (EMA), Denali (VDOT) | 340 dialysis patients with no access to treatment clinics |
| 2 | Environment | Acadia (DEQ), Zion (AEP), Banff (Governor's) | Coal ash pond structural risk before major rainfall |
| 3 | Health/Supply | Redwood (United Way), Bryce (Faith Network), Olympic (Supply Chain) | Opioid surge + MOUD medication crisis |
| 4 | Biosecurity | Sequoia (VT), Jasper (Biosecurity/CISA), Yosemite (Sheriff's) | Respiratory illness cluster in coal community |
| 5 | Environment/Health | Shenandoah (Montgomery Co), Yellowstone (WDBJ7), Yoho (Shentel) | Preliminary water contamination in 3 rural systems |

### Escalation Triggers (Negative — open if teams fail to respond)

> **Timing note:** `sim_time` values (e.g., `0:15`) are facilitator labels indicating when an inject fires in the simulated timeline. The `time_limit` field in `injects.csv` is what the dashboard uses to drive countdown timers; it is measured in minutes from phase start. Both refer to the same deadline — `sim_time` for narrative context, `time_limit` for the engine.

When an escalation trigger fires, **immediately mark the corresponding initial inject as `escalated`**. Each initial inject has explicit `RESPONSE COUNTS AS` clauses, one per required team. Award **partial credit per clause met before the trigger deadline**: each team that satisfied its own clause earns an equal proportional share of the initial inject's `points_resolve` (1/3 each for the standard 3-team initial injects in this phase). Teams that did not meet their clause earn 0. The escalation inject is then the live arc and is scored independently on its own merits.

| Trigger Condition | Inject | Severity |
|-------------------|--------|----------|
| No dialysis response by 0:15 | #6 — Three patients die; federal wrongful death inquiry opened | 5 |
| No coal ash response by 0:10 | #7 — Seepage confirmed; Clinch River do-not-use advisory mandatory | 5 |
| No MOUD response by 0:20 | #9 — Three MOUD clinics suspend; patients presenting in withdrawal | 4 |
| No water system response by 0:20 | #10 — Viral panic posts; incorrect boil-water advice spreading | 4 |
| No respiratory response by 0:25 | #8 — CDC notified; mine halted; workers walk out | 3 |

### Success Triggers (Positive — open if teams respond in time)

Open these injects as **facilitator reward** when the corresponding initial inject is marked resolved before the escalation trigger fires. Do not open both the escalation and success inject for the same arc.

**Scoring rule:** Every inject is scored independently. Award `points_resolve` for any inject that is resolved, regardless of whether it is an initial inject, escalation trigger, or success follow-on. Initial injects marked `escalated` award a proportional share of `points_resolve` per team that met its own `RESPONSE COUNTS AS` clause before the deadline (1/N where N = number of required teams); teams that did not meet their clause earn 0. **An inject in `escalated` state cannot be changed to `resolved` — it is permanently closed for full-points scoring.** Record all reasoning in progress.md.

| Condition | Inject | What it unlocks |
|-----------|--------|-----------------|
| Dialysis resolved before 0:15 | #11 — Regional medical transport model | Carilion/EMA/VDOT formalize lasting compact; bonus points |
| Coal ash stabilized before 0:10 | #12 — Downstream community trust window | DEQ/AEP/Governor sign monitoring compact with community; bonus points |
| MOUD stabilized before 0:20 | #13 — Recovery community mobilizes as asset | United Way/Faith/Supply Chain formalize peer navigator program; bonus points |
| Respiratory identified before 0:25 | #14 — Voluntary mine restart protocol | VT/CISA/Sheriff broker monitoring agreement; bonus points |
| Water advisory resolved before 0:20 | #15 — Public health authority restored | Montgomery Co/WDBJ7/Shentel build permanent early-warning system; bonus points |

---

## Constraints

### Time Constraints
- Phase duration: 60 minutes real-time = Days 16-25 simulated
- Dialysis patient window: 0:15 before preventable deaths
- Coal ash window: 0:10 before storm makes stabilization ineffective
- MOUD clinic collapse: 0:20 if no emergency dispensing protocol

### Budget Constraints
- Budgets carry over from Phase 4 end state
- Most teams have reduced budgets after Phase 4 expenditure
- Environmental stabilization (coal ash) is expensive ($500K); requires trade-offs
- Health actions carry high social returns but significant cost

### Resource Constraints
- Carilion staff exhausted from Phase 4; surge capacity near limit
- DEQ has limited field staff for simultaneous coal ash + water + air monitoring
- Transport capacity limited; VDOT corridors still recovering from Phase 4 protest disruption
- Community trust (faith networks, United Way) is high but organizational bandwidth strained

### Information Constraints
- Known: dialysis patients at risk, coal ash sensors alarming, water tests preliminary, overdose spike, respiratory cluster
- Unknown: coal ash structural timeline, water contamination source/extent, respiratory illness cause, MOUD supply timeline
- Unreliable: social media claims about mine collapse, water "poison," outbreak origins

### Political/Legal Constraints
- Precautionary water advisory triggers mandatory bottled water distribution (costly)
- Mine shutdown order requires Governor approval and creates economic backlash in coalfields
- Federal CDC involvement triggers notification requirements and may alarm public
- Environmental enforcement decisions have long-term legal liability implications
